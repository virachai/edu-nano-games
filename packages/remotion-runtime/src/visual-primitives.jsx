...
export function StateCard({ state, opacity = 1, scale = 1, label = "State" }) {
  return (
    ...
    >
      <div style={{ fontSize: 22, opacity: 0.65, marginBottom: 12 }}>
        {label}
      </div>
  ...
  return (
    ...
    >
      {changes.map((change, index) => {
        const label =
          typeof change === "string" ? change : (change?.path ?? "change");
        const hasDiff =
        ...
        return (
          <div
            key={`${label}-${index}`}
            style={{ fontSize: 24, marginTop: 8, opacity: 0.8 }}
          >
            {hasDiff
              ? `${label}: ${displayValue(change.before)} → ${displayValue(change.after)}`
              : label}
          </div>
        );
      })}
...
export function SceneTimeline({ scenes, frame }) {
  ...
  const scene =
    safeScenes.find(
      (candidate) =>
        frame >= candidate.frame.start && frame < candidate.frame.end
    ) ?? safeScenes[safeScenes.length - 1];
  if (!scene) return null;
  const localFrame = Math.max(0, frame - scene.frame.start);
  const progress = interpolate(
    localFrame,
    [0, Math.max(1, scene.frame.duration - 1)],
    [0, 1],
...
export function SemanticScene({ scene, progress }) {
  const enter = interpolate(progress, [0, 0.22, 1], [0, 1, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const scale = interpolate(progress, [0, 0.35, 1], [0.96, 1, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  ...
  return (
    ...
    >
      <div style={{ width: "100%", textAlign: "center" }}>
        ...
        >
          {scene.render?.label ?? scene.id}
        </div>
        <StateCard
          state={state}
          opacity={enter}
          scale={scale}
          label={scene.render?.phase ?? "semantic state"}
        />
