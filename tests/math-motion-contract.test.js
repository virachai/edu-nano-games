...
test('6174 execution compiles to deterministic valid Remotion scenes', () => {
  const result = executeRecipe(canonicalRecipe);
  const compilation = compileRemotionScenes(result.trace, { fps: 30, stepFrames: 15 });
  ...
  assert.equal(compilation.scenes.at(-1).frame.end, compilation.durationInFrames);
  ...
  const replay = compileRemotionScenes(result.trace, { fps: 30, stepFrames: 15 });
